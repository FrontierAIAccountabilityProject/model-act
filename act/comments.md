...
## Open issues — QUESTIONS FOR THE NEXT REVISION (v3.5)
...
An answer to an open item may become splice-ready language in the [v3.5 cure queue](../revision/proposals.md) before the revision is tagged. State-specific items are instructions for an adopting jurisdiction rather than unfinished universal text. Closed items remain visible so that the movement is checkable.
...
[the bill census](../appendix/bill-census.md).*
...
Comments on savings-clause construction. The counterweight is stated honestly: *Arizona v. United
...
([the 20 August sweep](../revision/standing-watch.md) § 1); **undecided**. That decision is the single most important pending development for SEC. 8 and
...
outbreak-scale stress test remains open at Open issues item 4.
...
disclosed*, not whether what is said must be true — chunk 2 §C.4, topic 2). Striking "any
...
floor — the Act's death-results minimum stays the bracketed [two] years, held at Open issues item
3(c) for a criminal-law scholar. The tier's injury *definition*, separately, leaves 21 C.F.R.
§ 803.3(w) for § 1365(h)(3)–(4) at v3.5 on an outside criminal-law reading (Open issues item 3;
v3.5 queue, Amendment 1) — the borrowing becomes one statute rather than two regimes. Each resulting death or serious injury is a separate offense with
...
tolling that runs from discovery) and the Comments regulations broke it a third way (Part 10.1's
...
floor. The bracketed figures are a default awaiting the evaluations researcher Open issues item 6
...
provable substrate. Preemption: the offense occupies chunk 2's fourth topic (record creation and
...
broader, and that check belongs to the conforming counsel of Open issues item 9.
...
## THE OPEN CITE-CHECK (Open issues ITEM 10)
...
- USSG § 5G1.2(d) against the 2025 Guidelines Manual print (archive-pinned at chunk 5 §B.1; the
  per-section current-manual page was JS-only/404 at the internal review).
...
Re-swept 20 August 2026 — [`audit/standing_watch_2026-08-20.md`](../revision/standing-watch.md):
...
- California 2025–26 regular session adjourns 31 Aug 2026 — bills not passed by adjournment
  die; the AI docket's final review follows within days.
...
manifest elements are the right minimum is squarely the security reviewer's terrain.
