...
**Between versions — 24 August 2026, the site wears the register.** No change to any tagged text, fact, or row. The site’s dress is rebuilt around the register the text already keeps: the accent goes to law-report maroon itself (the first scheme’s spruce was chosen as that maroon’s exact complement — the colour comes home), the reading page to warm paper under thick near-black ink at a size any reader manages, the body to the serif of the law reports. The sidebar becomes the dark apparatus panel — cream on ink, brass rail, the reading order numbered §1–§10 — and the top-right repository link is dressed as the stamp it is, its label shortened to “Authoritative record ↗” (the front page’s pointer to the top-right link remains true). One front-page addition, insertion-only, under the title: a dark panel carrying the Act’s effect in one sentence over a monospaced status strip — version, archive, DOI, licence, the bracketed compute floor — every item already stated on the page it sits on. The favicon keeps the § mark, recoloured to the scheme. Twelve rows untouched in both stated places; nav remains the ten exact paths; the theme stays pinned.
