...
**Thirty rows are missing counsel, and no amount of reading reduces that number.** It is the
...
**Forty-three rows are missing a document** — the largest level, and the cheapest to clear, because
finding a file needs no expertise. Four of them are the participants' own accounts of the 2026
incidents, which are the evidence base for this whole Act. Until those are read, that base rests on
journalism about them.

**Twenty-one rows need only a reading.** Both texts are on the shelf and the comparison has not been
made.
